...
def normalize_graph_features(dataset, method='dataset'):
    """
    高效的原地特征归一化

    Args:
        dataset: PyG Dataset对象
        method: 'dataset' (数据集级) 或 'graph' (单图级)

    Returns:
        mean_vector: [1, D] tensor (dataset模式) 或 None (graph模式)
    """
    ...
    if method == 'dataset':
        ...
        for data in dataset:
            if running_sum is None:
                running_sum = torch.zeros(data.x.size(1), device=data.x.device)
            running_sum += data.x.sum(dim=0)
            total_nodes += data.x.size(0)

        mean_vector = (running_sum / total_nodes).unsqueeze(0)

        # 原地减均值
        for data in dataset:
            data.x.sub_(mean_vector)
        ...
        # 单图级
        for data in dataset:
            graph_mean = data.x.mean(dim=0, keepdim=True)
            data.x.sub_(graph_mean)
...
def load_datasets_with_prompts(
        args,
    ...
    if normalize_features:
        ...
        for key, info in train_datasets.items():
            normalize_graph_features(info["dataset"], method=normalization_method)

        normalize_graph_features(target_info["dataset"], method=normalization_method)

    # DataLoaders
    target_loader = DataLoader(target_info["dataset"], batch_size=args.batch_size, shuffle=False)
    target_loader.prompt_key = target_info["prompt_key"]
    target_loader.name = args.dataset

    train_loaders = []
    for key, info in train_datasets.items():
        loader = DataLoader(info["dataset"], batch_size=args.batch_size, shuffle=True)
        loader.prompt_key = info["prompt_key"]
        loader.name = key
