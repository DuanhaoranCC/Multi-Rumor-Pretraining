...
class BiGCN_graphcl(torch.nn.Module):
    ...
    def process_data(self, data):
        ...
        # ---- 对称门控（alpha大 -> 加法更强；alpha小 -> 乘法更强）----
        # z = x + (1.0 - alpha) * (x * p_mul) + alpha * p_add
        # z = (1.0 - alpha) * (x * p_mul) + alpha * (x + p_add)
        # z = (1.0 - alpha) * x * (1 + p_mul) + alpha * (x + p_add)
        # z = x * p_mul + x + p_add
        z = x
    ...
    #     out = torch.cat([x1, x2], dim=0)  # [2N, D]
    #
    #     # logits: [2N, 2N]
    #     logits = (out @ out.t()) / T
    #
    #     # mask self-similarity by large negative
    #     mask = torch.eye(2 * N, device=logits.device, dtype=torch.bool)
    #     logits = logits.masked_fill(mask, -1e9)
    #
    #     # positive index for each anchor:
    #     # for i in [0..N-1], pos is i+N; for i in [N..2N-1], pos is i-N
    #     pos_idx = torch.arange(2 * N, device=logits.device)
    #     pos_idx = (pos_idx + N) % (2 * N)
    #
    #     loss = F.cross_entropy(logits, pos_idx, reduction="mean" if mean else "none")
    #     return loss
